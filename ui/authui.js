...
export const authUI = {
  AuthUI: class {
      ...
      start(containerId, config) {
        ...
        try {
          const res = await this.auth.signInWithPopup(new auth.GoogleAuthProvider());
        ...
        try {
          const res = await this.auth.signInWithPopup(new auth.FacebookAuthProvider());
          errorBox.textContent = "";
          let cRes = true;
          if (config && config.callbacks && config.callbacks.signInSuccessWithAuthResult) {
            let sUrl = null;
        ...
        try {
          const res = await this.auth.signInWithPopup(new auth.GithubAuthProvider());
          errorBox.textContent = "";
          let cRes = true;
          if (config && config.callbacks && config.callbacks.signInSuccessWithAuthResult) {
            let sUrl = null;
        ...
        if (config.signInOptions?.includes(auth.GoogleAuthProvider.PROVIDER_ID)) {
          box.appendChild(btnGoogle);
        }

        if (config.signInOptions?.includes(auth.FacebookAuthProvider.PROVIDER_ID)) {
          box.appendChild(btnFacebook);
        }

        if (config.signInOptions?.includes(auth.GithubAuthProvider.PROVIDER_ID)) {
        ...
        if (config.signInOptions?.includes(auth.EmailAuthProvider.PROVIDER_ID)) {
        ...
        loginBtn.onclick = async () => {
          try {
            const res = await this.auth.signInWithEmailAndPassword(emailInput.value, passwordInput.value);
            errorBox.textContent = "";
            let cRes = true;
          if (config && config.callbacks && config.callbacks.signInSuccessWithAuthResult) {
            let sUrl = null;
        ...
        registerBtn.onclick = async () => {
          try {
            const res = await this.auth.createUserWithEmailAndPassword(this.auth, emailInput.value, passwordInput.value);
            errorBox.textContent = "";
            let cRes = true;
            if (config && config.callbacks && config.callbacks.signInSuccessWithAuthResult) {
              let sUrl = null;

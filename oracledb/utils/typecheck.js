...
export function argCheck(value, message, throwNullError, expectedTypes = []) {
    if (value === null || value === undefined) {
        ...
        let error = new Error(message || formatMessage(errorMessages.valueCannotBeNull));
        error.status = 400;
        throw oracledbErrorHandler(error);
    }
    ...
    if (!expectedTypes.map(t => t.toLowerCase()).includes(actualType)) {
        let error = new Error(
            message || formatMessage(errorMessages.expectedType, expectedTypes.join(", "), actualType)
        );
        error.status = 400;
        throw oracledbErrorHandler(error);
    }
